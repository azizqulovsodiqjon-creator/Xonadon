"use strict";
  ...
  function init(){
    ...
    // Map page's own filter bar (segment + property type + pills) - a
    // second set of controls over the same shared filterState /
    // matchesFilters as the home toolbar, so filtering works whether
    // you set it up before opening the map or right there on it.
    document.querySelectorAll('#mapSegment button').forEach(function(b){
      b.addEventListener('click', function(){
        ...
        this.classList.add('active');
        filterState.deal = this.getAttribute('data-deal');
        var homeDealBtn = document.querySelector('#segment button[data-deal="'+filterState.deal+'"]');
        document.querySelectorAll('#segment button').forEach(function(x){ x.classList.toggle('active', x===homeDealBtn); });
        refreshMapMarkers();
        syncFilterUrl();
      });
    });
    ...
    mapTypeDropdown.querySelectorAll('button').forEach(function(opt){
      opt.addEventListener('click', function(e){
        ...
        document.getElementById('mapTypeLabel').textContent = this.textContent;
        mapTypeDropdown.querySelectorAll('button').forEach(function(b){ b.classList.toggle('active', b===opt); });
